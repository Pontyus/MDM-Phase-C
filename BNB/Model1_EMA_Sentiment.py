...
tweets_df = pd.read_csv('tweets_reddit_processed_sentiment.csv')
...
tweets_df2['Number_of_Tweets'] = tweets_df['Number_of_Tweets'].ewm(span=100).mean()  # rolling average on 3 data points
tweets_df2['Average_Number_of_Likes'] = tweets_df['Average_Number_of_Likes'].ewm(span=100).mean()
tweets_df2['Average_Number_of_Retweets'] = tweets_df['Average_Number_of_Retweets'].ewm(span=100).mean()
tweets_df2['Average_Number_of_Replies'] = tweets_df['Average_Number_of_Replies'].ewm(span=100).mean()
tweets_df2['Average_Twitter_Sentiment'] = tweets_df['Average_Twitter_Sentiment'].ewm(span=100).mean()
tweets_df2['Average_Number_of_Quotes'] = tweets_df['Average_Number_of_Quotes'].ewm(span=100).mean()
tweets_df2['Number_of_Reddit_posts'] = tweets_df['Number_of_Reddit_posts'].ewm(span=100).mean()
tweets_df2['Average_Reddit_Sentiment'] = tweets_df['Average_Reddit_Sentiment'].ewm(span=100).mean()
...
tweets_df = tweets_df.sort_values(by=['Date'], ascending=True)
prices_df = prices_df.sort_values(by=['unix'], ascending=True)
new_tweets = new_tweets.sort_values(by=['Date'], ascending=True)
new_prices = new_prices.sort_values(by=['unix'], ascending=True)

prices_df = prices_df.drop(prices_df.head(len(prices_df) - len(tweets_df)).index)
...
for row in tweets_df.itertuples():
    ...
    reddit_sentiments.append(row.Average_Reddit_Sentiment)
    twitter_sentiments.append(row.Average_Twitter_Sentiment)
...
df = pd.DataFrame({'Price': prices, 'Volume': volumes, 'Number_of_Tweets': tweet_number,
                   ...
                   'Number_of_Reddit_posts': post_number, 'Twitter_Sentiment': twitter_sentiments,
                   'Reddit_Sentiment': reddit_sentiments})
...
plt.plot(num_of_dates, prices, color='black', label='Actual Price', linewidth='2')
plt.plot(num_of_dates, april_predictions_linear, color='red', label='Predicted Price (Linear Model)', linewidth='2')
plt.plot(num_of_dates, april_predictions_dnn, color='blue', label='Predicted Price(DNN Model)', linewidth='2')
plt.legend(prop={'size': 30}, title='Price Predictions of BNB in April 2021', fontsize='30')
